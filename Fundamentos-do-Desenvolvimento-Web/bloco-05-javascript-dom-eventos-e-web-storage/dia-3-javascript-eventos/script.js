...
function adicionaLegenda (cor){
    ...
    divNova.classList.add("task")
    divNova.setAttribute("style", "background-color:" + cor)
    
    // pegar divmytasks
    let divMyTasks = document.querySelector(".my-tasks")

    //adicionar divnova como filho da div mytasks
...
function adicionaEvento () {
    ...
    for (let index = 0; index < tasksList.length; index += 1){
        tasksList[index].addEventListener("click", function(event){
            if(tasksList[index].getAttribute("class") == "task selected"){
